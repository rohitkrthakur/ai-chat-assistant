...
export async function POST(req: Request) {
  try {
    ...
    if (msgLower.includes('weather')) {
      ...
      if (!location) {
        aiResponse = 'Please specify a location, e.g., "weather of London".'
      } else {
    ...
    // Handle stock queries
      ...
      if (!symbol) {
        aiResponse = 'Please specify a stock symbol, e.g., "stock of AAPL".'
      } else {
        const res = await fetch(
        ...
        if (quote) {
          aiResponse = `${quote['01. symbol']} price: $${parseFloat(
            quote['05. price']
          ).toFixed(2)}, Change: ${parseFloat(quote['09. change']).toFixed(
            2
